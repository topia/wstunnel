      ...
      const handleReq = (request, connWrapperCb) => {
        ...
        return this.authenticate(httpRequest, (rejectReason, target, monitor) => {
          ...
          var tcpConn = net.connect({host, port, allowHalfOpen: false}, () => {
            ...
            log(`Client ${ip} via ${reprStreamAddress(httpRequest)} establishing ${request instanceof HttpTunnelReq ? 'http' : 'ws'} tunnel `+
              `to ${host}:${port} / ${reprStreamAddress(tcpConn)}`);
      ...
      this.wsServer.on('request', req => {
        ...
        if (wsReconnTimeout && (sessid = req.httpRequest.headers['x-htunsess'])) {
          ...
          if (sess === null) {
            ...
            sess.setTimeout(wsReconnTimeout);
            closed = false;
            sess.on('close', (...args) => {
              if (closed) {
                return;
              ...
              closed = true;
              log(`finished session ${sessid}: ${args}`);
