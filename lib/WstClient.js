...
const reprStreamAddress = require('./reprStreamAddress');
  ...
  */
  ...
  startStdio(wsHostUrl, remoteAddr, optionalHeaders, cb) {
    ...
    const bind = s => {
      ...
      s.on('error', err => {
        log(`connection fatal error: ${err}\r`);
      });
      s.on('informError', err => {
        log(`connection error: ${err}\r`);
      });
      s.on('handshakeError', err => {
        log(`handshake error: ${err}\r`);
        process.exit(2);
      });
      s.on('bound', ws => {
        log(`connected: ${reprStreamAddress(ws)}\r`);
      });
      s.on('detached', ws => {
        log(`detached from: ${reprStreamAddress(ws)}\r`);
      });
      process.on('SIGUSR1', () => {
        log(`SIGUSR1 received. start reconnection\r`);
        s.queueReconnect();
      });
      return s;
    };
  ...
  _wsConnect(wsHostUrl, remoteAddr, optionalHeaders, cb) {
    ...
    let connector = insideCb => {
      ...
      wsClient.on('connectFailed', error => insideCb(error));
    ...
    if (this.wsReconnTimeout) {
      ...
      this.streamer.setTimeout(this.wsReconnTimeout)
    }
