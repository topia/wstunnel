...
module.exports = (WsBufferedStream = class WsBufferedStream extends stream.Duplex {
  ...
  _write(chunk, encoding, callback) {
    ...
    if (this.sendQueue.length || this.ws === null) {
      ...
      this.ws.sendBytes(chunk, (err) => {
        // catch any errors when reconnectable, because it causes 'error' event and triggers `unpipe`.
        if (typeof this.reconnector === 'function' && err) {
          this.onError(err);
          callback();
        } else {
          callback(err);
